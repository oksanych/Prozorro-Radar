      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-slate-400 mt-1">
            ...
            {case_.updated_at && <span className="text-slate-500"> · Updated {formatDate(case_.updated_at)}</span>}
          </div>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0 pt-2 sm:pt-0 border-t border-slate-700 sm:border-0">
          <Link
            href={`/cases/${case_.id}`}
            className="text-xs px-3 py-2.5 sm:py-1.5 border border-slate-600 rounded text-slate-300 hover:border-slate-400 hover:text-slate-100 transition-colors"
          >
            Open →
          </Link>
          <a
          ...
          <button
            ...
            disabled={deleting}
            className="text-xs px-3 py-2.5 sm:py-1.5 border border-slate-600 rounded text-red-400 hover:border-red-500 hover:text-red-300 transition-colors disabled:opacity-50"
          >
            Delete
          </button>
        </div>
